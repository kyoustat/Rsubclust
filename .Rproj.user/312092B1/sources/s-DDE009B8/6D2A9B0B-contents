...
#' \item{data}{an \eqn{( n*(nl+np) \times 3)} data matrix.}
#' \item{class}{length-\eqn{n*(nl+np)} vector for class label.}
#' }
#' 
#' @examples 
...
#' 
#' @export
simLP <- function(n=100, nl=2, np=1, iso.var=0.1){
  ...
  for(k in 1:K){
    sigma0.true.list[[k]]=stats::runif(k.true[k],isotropic.var,5.1)
  }
  Sigma0.true.list = list()
  for(k in 1:K){
